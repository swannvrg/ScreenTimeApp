...
import { Card, SectionLabel, StatTile, ProgressBar, AlertBanner, StatusDot } from '@/components/ui'
import { RefreshCw, Settings } from 'lucide-react'
import Link from 'next/link'

const C = {
  accent: '#6ee7b7', accentB: '#3b82f6',
  danger: '#ff6b8a', warn: '#fbbf24',
  text: '#f0f0ff', muted: 'rgba(255,255,255,0.4)',
...
export default function DashboardPage() {
  ...
  return (
    <main style={{
      ...
      padding: '32px 16px 80px', maxWidth: 680, margin: '0 auto',
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div className="fade-up" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32 }}>
          <div>
            <p style={{ fontSize: 12, fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', color: C.muted, marginBottom: 6 }}>Gamification</p>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
            <button onClick={refetch} disabled={loading} style={{ background: 'rgba(255,255,255,0.06)', border: `1px solid ${C.border}`, borderRadius: 12, padding: '8px 12px', cursor: 'pointer', color: C.muted }}>
        ...
        {/* ── Status ── */}
        <div className="fade-up s1" style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: C.muted, marginBottom: 24 }}>
          <StatusDot status={loading ? 'loading' : error ? 'error' : 'ok'} />
          <span>{loading ? 'Chargement…' : error ? `Erreur : ${error}` : `${data?.raw.length ?? 0} logs · ${lastUpdate}`}</span>
        ...
        <div className="fade-up s2" style={{ marginBottom: 12 }}>
          <Card style={{ background: 'linear-gradient(135deg, rgba(110,231,183,0.1), rgba(59,130,246,0.08))', border: '1px solid rgba(110,231,183,0.2)', position: 'relative', overflow: 'hidden', padding: '28px 24px' }}>
            <div style={{ position: 'absolute', top: -40, right: -40, width: 180, height: 180, borderRadius: '50%', background: 'radial-gradient(circle, rgba(110,231,183,0.12) 0%, transparent 70%)', pointerEvents: 'none' }} />
            <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: C.muted, marginBottom: 16 }}>Solde du jour</p>
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, marginBottom: 20 }}>
              <span style={{
                fontSize: 96, fontWeight: 800, lineHeight: 0.9,
                background: !data ? 'none' : data.solde >= 0 ? `linear-gradient(135deg, ${C.accent}, ${C.accentB})` : 'linear-gradient(135deg, #ff6b8a, #ff2d55)',
              ...
              {data && <span style={{ fontSize: 22, fontWeight: 600, color: C.muted, paddingBottom: 10 }}>min</span>}
            </div>
            <div style={{ display: 'flex', gap: 16 }}>
              <div style={{ flex: 1, background: 'rgba(110,231,183,0.08)', border: '1px solid rgba(110,231,183,0.15)', borderRadius: 12, padding: '10px 14px' }}>
                ...
                <p style={{ fontSize: 22, fontWeight: 700, color: C.accent }}>{data?.earned ?? '—'}</p>
              </div>
              <div style={{ flex: 1, background: 'rgba(255,107,138,0.08)', border: '1px solid rgba(255,107,138,0.15)', borderRadius: 12, padding: '10px 14px' }}>
                ...
                <p style={{ fontSize: 22, fontWeight: 700, color: C.danger }}>{data?.spent ?? '—'}</p>
              </div>
            </div>
          </Card>
        </div>
        ...
        {/* ── Stats 2×2 ── */}
        <div className="fade-up s3" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 4 }}>
        ...
        <div className="fade-up s4">
          <Card>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: C.muted }}>Gagné vs Dépensé</span>
              <div style={{ display: 'flex', gap: 14 }}>
                {[['#6ee7b7', '⚡ Gagné'], ['#ff6b8a', '📱 Dépensé']].map(([c, l]) => (
            ...
            {data ? <WeekChart days={data.weekDays} /> : (
              <div style={{ height: 140, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.muted, fontSize: 13 }}>Pas de données</div>
            )}
          </Card>
        </div>
        ...
        <div className="fade-up s5">
          <Card>
            {!data || data.todayTasks.length === 0 ? (
              ...
              <div key={t.name} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: i < data.todayTasks.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                <span style={{ flex: 1, fontSize: 14, color: C.text, fontWeight: 500 }}>{t.name.replace(/_/g, ' ')}</span>
                <span style={{ fontSize: 12, color: C.muted }}>×{t.count}</span>
                <span style={{ fontSize: 13, fontWeight: 600, color: C.accent, background: 'rgba(110,231,183,0.12)', padding: '4px 10px', borderRadius: 8 }}>+{t.total} min</span>
              </div>
            ))}
          </Card>
        </div>
        ...
        <div className="fade-up s6">
          <Card>
            {!data ? (
              ...
              // Afficher date lisible depuis ISO
              const [y, m, d] = r.date.split('-')
              const dateDisplay = `${d}/${m}/${y}`
              return (
                <div key={r.id} style={{ display: 'grid', gridTemplateColumns: '76px 46px 1fr 52px', gap: 8, padding: '10px 0', alignItems: 'center', borderBottom: i < data.recent.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                  <span style={{ fontSize: 12, color: C.muted }}>{dateDisplay}</span>
                  <span style={{ fontSize: 12, color: C.muted }}>{r.heure}</span>
                  <span style={{ fontSize: 13, color: C.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.nom_task.replace(/_/g, ' ')}</span>
                  <span style={{ fontSize: 13, fontWeight: 600, textAlign: 'right', color: spend ? C.danger : C.accent }}>{spend ? '-' : '+'}{r.time}</span>
                </div>
              )
            })}
          </Card>
