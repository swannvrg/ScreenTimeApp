...
export default function DashboardPage() {
  const { scriptUrl, isAuthenticated, logout } = useAuth()
  const router = useRouter()
  useEffect(() => { if (!isAuthenticated) router.replace('/login') }, [isAuthenticated, router])
  const { data, loading, error, lastUpdate, refetch, budget } = useScreenData(scriptUrl)
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div className="fade-up" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32 }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
            <button onClick={refetch} disabled={loading} style={{ background: 'rgba(255,255,255,0.08)', border: `1px solid ${C.border}`, borderRadius: 12, padding: '8px 12px', cursor: 'pointer', color: C.muted, transition: 'all 0.2s' }}>
              <RefreshCw size={15} style={{ display: 'block', animation: loading ? 'spin 0.8s linear infinite' : 'none' }} />
            </button>
            <button onClick={logout} style={{ background: 'rgba(255,107,138,0.1)', border: '1px solid rgba(255,107,138,0.2)', borderRadius: 12, padding: '8px 12px', cursor: 'pointer', color: C.danger, transition: 'all 0.2s' }}>
              <LogOut size={15} style={{ display: 'block' }} />
            </button>
          </div>
        </div>
          ...
          <Card style={{
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, marginBottom: 20 }}>
              <span style={{
                ...
                background: !data ? 'none' : data.solde >= 0
              ...
              <div style={{ display: 'flex', gap: 14 }}>
                {[['#6ee7b7','⚡ Gagné'],['#ff6b8a','📱 Dépensé']].map(([c,l]) => (
                  <span key={l} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: C.muted }}>
                    <span style={{ width: 8, height: 8, borderRadius: 3, background: c, display: 'inline-block' }} />
                    {l}
                  </span>
                ))}
              </div>
              ...
              <div>
                {data.todayTasks.map((t, i) => (
                  <div key={t.name} style={{
                    ...
                    borderBottom: i < data.todayTasks.length - 1 ? `1px solid ${C.border}` : 'none',
                  }}>
                    <span style={{ flex: 1, fontSize: 14, color: C.text, fontWeight: 500 }}>
                      {t.name.replace(/_/g, ' ')}
                    </span>
                    <span style={{ fontSize: 12, color: C.muted, flexShrink: 0 }}>×{t.count}</span>
                    <span style={{
                      fontSize: 13, fontWeight: 600, color: C.accent,
                      background: 'rgba(110,231,183,0.12)', padding: '4px 10px', borderRadius: 8,
                      flexShrink: 0,
                    }}>
                      +{t.total} min
                    </span>
        ...
        <div className="fade-up s6">
          <Card>
            {!data ? (
              ...
              <div>
                {data.recent.map((r, i) => {
                  const spend = r.nomTask === 'Consommation temps'
                  return (
                    <div key={i} style={{
                      display: 'grid', gridTemplateColumns: '90px 52px 1fr 48px',
                      gap: 8, padding: '10px 0', alignItems: 'center',
                      borderBottom: i < data.recent.length - 1 ? `1px solid ${C.border}` : 'none',
                    }}>
